(* Processes the plain dungeon map to add features *)
...
begin
  ...
  begin
    for c := 1 to globalutils.MAXCOLUMNS do
      ...
      begin
        tileCounter := 0;
        if (dungeon.dungeonArray[r - 1][c] <> '.') then // NORTH
          tileCounter := tileCounter + 1;
        if (dungeon.dungeonArray[r][c + 1] <> '.') then // EAST
          tileCounter := tileCounter + 4;
        if (dungeon.dungeonArray[r + 1][c] <> '.') then // SOUTH
          tileCounter := tileCounter + 8;
        if (dungeon.dungeonArray[r][c - 1] <> '.') then // WEST
